...
/* ---------- JSXText-гигиена: ноль «голых» комментариев в дереве рендера ----------
 *
 * REGRESSION (независимое ревью cca9d2e): в позиции детей JSX блок
 * /* ... *\/ — НЕ комментарий: TypeScript парсит его как JSXText, tsc
 * молчит, а React рендерит ~470 символов служебного текста над графиком.
 * Комментарий в дереве рендера обязан быть JSX-комментарием {/* ... *\/.
 *
 * Проверка по СЫРОМУ исходнику (не после stripComments — тот вырезает
 * и «голые» блоки, из-за чего прежние regex-гарды класс пропускал):
 * от корня return-разметки до конца файла снимаем только корректные
 * {/* ... *\/ и требуем нулевого остатка /* или *\/ — то есть ни
 * «голых» комментариев, ни потерянных маркеров в JSXText.
 */
{
  const JSX_ROOT_ANCHOR = '<div className="chartCard">';
  const jsxRootStart = CHART_SRC.indexOf(JSX_ROOT_ANCHOR);

  ok(
    jsxRootStart !== -1,
    "chart: якорь дерева рендера найден (<div className=\"chartCard\">)"
  );

  const jsxTree = CHART_SRC.slice(jsxRootStart).replace(
    /\{\/\*[\s\S]*?\*\/\}/g,
    ""
  );

  eq(
    (jsxTree.match(/\/\*|\*\//g) ?? []).length,
    0,
    "chart: в дереве рендера нет маркеров /* или */ вне {/* */} — «голый» комментарий JSX-детями был бы видимым текстом над графиком"
  );
}
