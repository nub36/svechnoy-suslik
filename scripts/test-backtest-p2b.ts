 ...
 *
 * P2AB INTEGRATION (12.09.2026, база — финальная P2-A 6f1977e4 / p2a-1.2.0):
 * - §18: bridge-тест «P2-B data plane → validateBars/runBacktest p2a-1.2.0»
 *   (metadata.contractVersion === BACKTEST_CONTRACT_VERSION, согласованность
 *   таймфрейма и канонической сетки) + negative control на испорченный порядок;
 * - §18b: D2/H1 boundary-регрессия — сигнал до границы, вход НА границе,
 *   «отравленное» будущее не должно давать ложного lookahead. На старой
 *   p2a-1.0.0 эти же ассерты падают (дифференциальный контроль closure),
 *   при этом гарантия не переоценивается: только context-канал + диагностика.
 */
...
(async () => {
  ...
  /* ------------------------------------------------------------------ */
  /* 18. P2AB INTEGRATION — BRIDGE + D2/H1 BOUNDARY REGRESSION            */
  /* ------------------------------------------------------------------ */
  /* Интеграция принятого P2-B data plane с финальной P2-A p2a-1.2.0      */
  /* (база интеграции — ровно 6f1977e4). Данные синтетические, БД нет.    */

  // (a) Bridge: P2-B fetch → P2-B adapter → p2a-1.2.0 validateBars/runBacktest
  const bridgeRows = makeSequentialRows(10, "1h", 24, T0);
  const bridgeFetch = await fetchCandlesPaginatedV2({
    marketId: 10,
    timeframe: "1h",
    from: new Date(T0),
    to: new Date(T0 + 24 * H1),
    deps: makeV2Deps(bridgeRows) as any,
    pageSize: 10,
  });
  ok(bridgeFetch.rows.length === 24, "P2AB bridge: P2-B fetch вернул 24 строки");
  ok(bridgeFetch.meta.pagesFetched === 3, "P2AB bridge: 3 страницы (10/10/4)");

  const bridgeBars = candleRowsToBacktestBars(bridgeFetch.rows);
  const bridgeValidation = validateBars(bridgeBars, BACKTEST_DEFAULTS);
  ok(bridgeValidation.ok, `P2AB bridge: p2a-1.2.0 validateBars принимает P2-B данные (${bridgeValidation.errors.join("; ")})`);
  eq(bridgeValidation.timeframeMs, H1, "P2AB bridge: шаг сетки P2-A = 1h");
  eq(bridgeValidation.gridGaps, 0, "P2AB bridge: пропусков сетки нет");
  eq(bridgeValidation.firstBarTime, T0, "P2AB bridge: первый бар — начало окна");
  eq(bridgeValidation.lastBarTime, T0 + 23 * H1, "P2AB bridge: последний бар — 23-й час");

  // Согласованность канонической сетки P2-B и P2-A метаданных
  const bridgeCoverage = computeMarketCoverage(10, "1h", bridgeBars, H1, {
    from: new Date(T0),
    to: new Date(T0 + 24 * H1),
  });
  ok(bridgeCoverage.requestedAlignment!.isAligned, "P2AB bridge: окно выровнено по канонической сетке");
  eq(bridgeCoverage.availableInRequestedRange, bridgeBars.length, "P2AB bridge: канонические слоты = числу баров");
  ok(
    bridgeBars.every((bar) => bar.time % H1 === 0),
    "P2AB bridge: все time баров лежат на канонической сетке 1h"
  );

  const bridgeProvider: SignalProvider = (context) => {
    if (context.index < 2) return null;
    const prev = context.barAt(context.index - 1);
    if (context.bar.close > prev.close) {
      return entryDecision("LONG", context.bar.low, context.bar.high + 2, "bridge", ["p2ab-bridge"]);
    }
    return null;
  };
  const bridgeRun = runBacktest({ bars: bridgeBars, signals: bridgeProvider });
  ok(bridgeRun.ok, `P2AB bridge: p2a-1.2.0 runBacktest принимает P2-B бары (${bridgeRun.ok ? "" : bridgeRun.errors.join("; ")})`);
  if (bridgeRun.ok) {
    eq(
      bridgeRun.result.metadata.contractVersion,
      BACKTEST_CONTRACT_VERSION,
      "P2AB bridge: metadata.contractVersion = BACKTEST_CONTRACT_VERSION"
    );
    eq(bridgeRun.result.metadata.timeframeMs, H1, "P2AB bridge: metadata.timeframeMs согласован с сеткой P2-B");
    eq(bridgeRun.result.metadata.barsCount, bridgeBars.length, "P2AB bridge: metadata.barsCount = числу P2-B баров");
    eq(bridgeRun.result.metadata.firstBarTime, T0, "P2AB bridge: metadata.firstBarTime согласован");
    eq(bridgeRun.result.metadata.gridGaps, 0, "P2AB bridge: metadata.gridGaps = 0");
  }

  // Отрицательный контроль bridge-теста: испорченный P2-B порядок обязан падать в P2-A валидации
  const reversedBridgeBars = candleRowsToBacktestBars([...bridgeFetch.rows].reverse());
  ok(
    !validateBars(reversedBridgeBars, BACKTEST_DEFAULTS).ok,
    "P2AB bridge (neg): обратный порядок P2-B данных отвергается validateBars (мост не вырожден)"
  );

  // (b) D2/H1 boundary regression: сигнал до границы, вход НА границе,
  //     «отравленное» будущее не должно давать ложного сообщения о lookahead.
  const d2Bars: BacktestBar[] = Array.from({ length: 60 }, (_, index) => ({
    time: T0 + index * H1,
    open: 100 + index,
    high: 101.5 + index,
    low: 99 + index,
  ...
  const d2Provider: SignalProvider = (context) => {
    const prev = context.index > 0 ? context.barAt(context.index - 1) : null;
    if (prev && context.bar.close > prev.close) {
      return entryDecision("LONG", context.bar.low, context.bar.high + 2, "d2", ["d2-boundary"]);
    }
    return null;
  };
  const d2Invariance = assertDecisionInvariance({ bars: d2Bars, provider: d2Provider, boundaryIndex: 30 });
  ok(
    d2Invariance.ok,
    `D2/H1 boundary: вход на границе не даёт ложного lookahead (${d2Invariance.errors.join("; ")})`
  );
  eq(d2Invariance.comparedDecisions, 30, "D2/H1 boundary: сравнен весь префикс решений");

  const d2Baseline = runBacktest({ bars: d2Bars, signals: d2Provider });
  const d2Poisoned = runBacktest({ bars: poisonFutureBars(d2Bars, 30, 3.5), signals: d2Provider });
  ok(d2Baseline.ok && d2Poisoned.ok, "D2/H1 boundary: оба прогона успешны");
  if (d2Baseline.ok && d2Poisoned.ok) {
    const preRecords = (items: readonly { index: number }[]) =>
      JSON.stringify(items.filter((item) => item.index < 30));

    ok(
      d2Baseline.result.rejectedSignals.every(
        (item) => !RETIRED_REJECT_REASONS.includes(item.reason)
      ),
      "D2/H1 boundary: p2a-1.2.0 не порождает отказы, причина которых — бар входа (RETIRED_REJECT_REASONS)"
    );
    ok(
      preRecords(d2Baseline.result.rejectedSignals) === preRecords(d2Poisoned.result.rejectedSignals),
      "D2/H1 boundary: rejected до границы идентичны (нет записи с индексом сигнала, вызванной баром входа)"
    );
    ok(
      preRecords(d2Baseline.result.skippedSignals) === preRecords(d2Poisoned.result.skippedSignals),
      "D2/H1 boundary: skipped до границы идентичны"
    );
    ok(
      JSON.stringify(d2Baseline.result.trades.filter((t) => t.exitIndex < 30)) ===
        JSON.stringify(d2Poisoned.result.trades.filter((t) => t.exitIndex < 30)),
      "D2/H1 boundary: сделки, закрытые до границы, идентичны"
    );
  }
  // Отрицательный контроль: диагностика не вырождена — реальный context-чит ловится.
  const d2Cheater: SignalProvider = (context) => {
    context.barAt(context.index + 4);
    return null;
  };
  ok(
    assertDecisionInvariance({ bars: d2Bars, provider: d2Cheater, boundaryIndex: 30 }).ok === false,
    "D2/H1 boundary (neg): реальное чтение будущего через контекст по-прежнему отвергается"
  );
