 ...
 *
 * Проводка пинится БЕЗ рантайм-шва (hardening #3): тест читает исходник
 * `run.ts` и проверяет, что `runExperiment` ВЫЗЫВАЕТ эту цепочку и
 * отвергает запись при непустом результате (`stage="invariant"`), а
 * мутационный контроль вне дерева удаляет вызов и показывает, что пин
 * падает. Это регрессионный/мутационный пин, а не формальное
 * доказательство; продакшн-хуков и изменяемого модульного состояния здесь
 * нет.
 *
