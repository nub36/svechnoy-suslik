...
**Статус P2-A (фундамент, 12.09.2026).** Реализован чистый детерминированный слой `lib/backtest/*`: контракт и зафиксированная политика исполнения, строгая валидация входа, издержки (слиппедж/комиссия), движок исполнения, метрики, хронологическое разбиение TRAIN/VALIDATION/OOS, каноническая сериализация с sha256-отпечатками и переиспользуемый инструмент доказательства no-lookahead. Три набора тестов: 260 + 123 + 108 проверок. Подробности — §43. Статус: **IMPLEMENTED / PENDING INDEPENDENT ADVERSARIAL AUDIT** — аудит ведёт отдельный агент; если он найдёт дефекты, они исправляются отдельным коммитом (автоматический rebase последующих этапов запрещён).

**Статус P2-C (слой эксперимента и отчёта, 12.09.2026).** Реализован чистый детерминированный слой `lib/experiment/*` (6 модулей): типизированный контракт эксперимента, идентичность конфигураций и отпечатки, дословная проекция метрик P2-A по сегментам, детерминированное сравнение ВСЕХ объявленных конфигураций, явное ранжирование/выбор только по TRAIN или VALIDATION, валидаторы хронологии/идентичности/утечек/изоляции OOS и прогон в памяти. Три набора тестов: 212 + 225 + 174 проверки. Подробности — §44. Это НЕ оптимизатор: параметры не подбираются, «лучшая» конфигурация по умолчанию не выбирается, проигравшие из отчёта не удаляются.

**Статус P2-B (данные/runner/UI).** Разрабатывается ОТДЕЛЬНЫМ агентом и в эти коммиты не входит: P2-A и P2-C от P2-B не зависят, БД не читают и не пишут. Интеграция P2-C с реальными данными и сохранением отчётов — отдельный этап, требующий PostgreSQL и одобрения владельца.
...
Что закрыто P2-C (без БД, в памяти, детерминированно):
- формализация TRAIN/VALIDATION/OOS на уровне эксперимента (переиспользуется P2-A `chronologicalSplit`, второго алгоритма разбиения нет)
- отчёт по каждому сегменту: сделки, gross/net PnL, win rate, PF + состояние, expectancy, avg/median R, обе базы просадки, серии убытков, неоднозначности/гэпы/таймауты, cannot-evaluate, пропуски и отказы с причинами (всё — дословная проекция результата P2-A, ничего не пересчитывается)
- сравнение конфигураций: все объявленные варианты присутствуют, отклонённые — с причиной, порядок представления не зависит от результата
- воспроизводимость: каноническая форма отчёта и три sha256-отпечатка (вход, результаты, отчёт) на P2-A-сериализаторе
- защита от утечек и подмен: хронология, непересечение, идентичность сегмента/конфигурации/рынка/таймфрейма, изоляция OOS от выбора

Что по-прежнему НЕ сделано (P2-B и далее; для P2-B нужны реальная БД и отдельное одобрение):
- Sharpe/Sortino, walk-forward, Monte-Carlo, устойчивость по активам/таймфреймам/режимам рынка (в P2-A метрик нет, а пересчитывать их в P2-C запрещено правилом «P2-A — источник истины»)
- подбор и оптимизация параметров, автоматический выбор «лучшей» стратегии как продукт, генерация сеток конфигураций
- подключение к реальным данным (PostgreSQL/Prisma), runner, сохранение прогонов и отчётов, UI «Тестер стратегий»
...
Заявлять прибыльность по-прежнему нельзя: P2-A даёт измеряемость и семантику, P2-C — честное сравнение и воспроизводимость, а не результат какой-либо стратегии.
...
9. **Гэп через уровень.** Исполнение по OPEN (для SL хуже уровня, для TP лучше), не по уровню: цены, которой не было в момент входа в бар, не существует. `gapThrough` помечается, слиппедж применяется и к гэповому исполнению. R при гэпе честно уходит за −1 (пример теста: −1.4).
...
4. MTM-просадка не резервирует будущую комиссию выхода (чуть оптимистичнее «полной» оценки), но при этом консервативнее реализованной базы; обе публикуются раздельно.
...
**Что дальше.** P2-B (подключение реальных данных, walk-forward, оптимизация, runner/UI) требует реальной PostgreSQL, учётных данных, записей в БД и отдельного одобрения владельца — поэтому P2-A завершён и СТОП: P2-B не начат, ничего не развёрнуто.
...
44. P2-C — EXPERIMENT & REPORT FOUNDATION: ЧИСТЫЙ ДЕТЕРМИНИРОВАННЫЙ СЛОЙ СРАВНЕНИЯ КОНФИГУРАЦИЙ БЕЗ БД И БЕЗ ОПТИМИЗАЦИИ (12.09.2026)
==================================================

**Хронология и baseline (проверено git, не «на глаз»):**
- Родитель ЭТОГО коммита — ровно `096e13d2a5a526d304b0448d987afdc6abf09ce2` (P2-A, §43), родитель которого `0d4b652edb8c6cf92df3df5cef3c68fc361106a1`. Без amend, без rebase, без rewrite: ОДИН новый коммит поверх принятого в работу состояния.
- Статусы на момент начала P2-C: `0d4b652` — **IMPLEMENTED / PENDING INDEPENDENT VPS ACCEPTANCE** (ручная проверка графика на VPS :3001 выполняется владельцем отдельно); P2-A `096e13d` — **IMPLEMENTED / PENDING INDEPENDENT ADVERSARIAL AUDIT** (аудит ведёт другой агент). P2-C аудита не ждал и не подменял: если аудит найдёт дефекты P2-A, они исправляются ОТДЕЛЬНЫМ коммитом, автоматический rebase/адаптер этого этапа владельцем запрещён и не выполнялся.
- **P2-B (реальные данные, БД, runner, UI) разрабатывается отдельным агентом и в этот коммит НЕ входит.** P2-C от P2-B не зависит: ни одного импорта, ни одного обращения к БД, ни одного адаптера «на будущее».
- Ветка — только `arena/01a09406-svechnoy-suslik`; `main` не тронут. `edf3732da81…` (Signal Engine dry-run) НЕ является предком: `git merge-base --is-ancestor edf3732 HEAD` → exit 1 (проверено до и после коммита).
- Защищённые области не изменены (0 строк в диффе): `components/chart/*`, `lib/smc/*`, `lib/strategies/*`, `lib/chart/*`, `app/api/chart/smc/route.ts`, `prisma/*`, `package.json`, `package-lock.json`, `lib/signals`, `signal-worker`. **`lib/backtest/*` (P2-A) не редактировался вовсе** — слой P2-C только импортирует его.

**Что такое P2-C (границы этапа).** P2-C — ФУНДАМЕНТ ЭКСПЕРИМЕНТА И ОТЧЁТА: формализация TRAIN/VALIDATION/OOS на уровне набора конфигураций, дословная проекция метрик P2-A по сегментам, детерминированное сравнение ВСЕХ объявленных конфигураций, воспроизводимость (каноническая форма + отпечатки) и чистые валидаторы утечек/идентичности/изоляции OOS. Это НЕ оптимизатор и НЕ тестер стратегий как продукт: параметры не подбираются, сетки конфигураций не генерируются, «лучшая» стратегия по умолчанию не выбирается, проигравшие конфигурации из отчёта не удаляются, Sharpe/Sortino/walk-forward/Monte-Carlo не считаются, БД/сеть/workers/файлы-персистентность не используются.

**Архитектура (6 модулей, все — чистые функции; 3 072 строки кода + 4 573 строки тестов):**
- `lib/experiment/contract.ts` (707 строк) — политика слоя (14 пунктов в шапке файла) и вся типовая поверхность: `ExperimentSubject` (стратегия slug+version, рынок asset/exchange/timeframe/timeframeMs, `DataRange`), `VariantDefinition`/`ResolvedVariant`, `PresentationOrderPolicy`, `SelectionStage = "TRAIN" | "VALIDATION"` (OOS в типе отсутствует физически), `RankingCriterion` (выведен компилятором из числовых ключей `EvidenceMetrics` — рассогласование критерия и метрики ломает сборку), `RANKING_CRITERION_DIRECTION`, `SelectionPolicy`, `SegmentReport` (60 полей дословной проекции, включая `provenance: "p2a-metrics-verbatim"`), `EvidenceMetrics`/`EvidenceEntry`/`EvidenceBlocks`, `RankingRecord` (`oosConsulted: false` как тип), `SelectionRecord`, `ExperimentCounts`, `OosIsolationRecord`, `ComparisonRow`/`ComparisonView`, `ExperimentRecord`, `ExperimentInput`, `ExperimentFailureStage`. Константы: `EXPERIMENT_CONTRACT_VERSION = "p2c-1.0.0"`, `EXPERIMENT_LAYER_NAME = "suslik-experiment"`, `DEFAULT_SELECTION_POLICY = { kind: "none" }`.
- `lib/experiment/identity.ts` (420) — `describeDataRange` (вычисляется ИЗ БАРОВ: `barsCount`, времена первого/последнего бара, `barsFingerprint` = P2-A `fingerprintBars`), `describeSubject`/`validateSubject`/`fingerprintSubject`, `describeSignalSource` (provider-форма ОБЯЗАНА иметь `signalSourceId`; список → длина + отпечаток), `fingerprintParams`, `configurationIdOf`, `resolveVariants` (отказ варианта не прерывает разрешение остальных; коды причин: `invalid-variant`, `invalid-config`, `missing-signal-source-id`, `duplicate-configuration-id`), `fingerprintExperimentInput`.
- `lib/experiment/report.ts` (487) — `projectSegmentReport` (дословная проекция результата P2-A: границы берутся из `metadata`, группировки `skippedByReason`/`rejectedByReason` строятся из записей P2-A, `resultFingerprint` = P2-A `fingerprintResult`), `projectEvidence`, `buildEvidenceBlocks` (ТРИ разделённых блока), `buildComparison` (все варианты → строки, `reportFingerprint`, `canonicalReport`), `canonicalExperiment`/`fingerprintExperiment`, `fingerprintSegmentResults`, `formatExperimentReport` (детерминированный текстовый вид: без дат, без случайных идентификаторов, `null` показывается как `null`/`n/a`, OOS помечен «в выборе НЕ участвует»).
- `lib/experiment/selection.ts` (370) — `validateOrderPolicy`/`validateSelectionPolicy` (stage=OOS отклоняется), `applyPresentationOrder` (доступны только `inputOrder` и `configurationId` — результат-зависимая сортировка невыразима), `rankEvidence` (принимает ТОЛЬКО блок своей стадии), `assertRankingIndependentOfOos` (пересчёт записанного порядка из сохранённых свидетельств и сверка байт-в-байт), `buildSelectionRecord`.
- `lib/experiment/validate.ts` (530) — `assertSplitChronological` (стыковка, полное покрытие, непустота, warmup только в прошлое, попарное непересечение, сверка `barsCount`), `assertResultSelfConsistency` (ловит грубую подделку: `metrics.trades` против числа записей, `equityCurve = trades + 1`, суммы `exitReasonCounts` и `decisionCounts`, `finalEquity = initialEquity + totalNetPnl`, `wins + losses + breakeven = trades`, перекрытие позиций), `assertSegmentIdentity` (версия контракта P2-A, сегмент, границы окна, `barsCount`, `barsFingerprint`, `timeframeMs`, границы диапазона, `configFingerprint`), `validateSubmittedSegmentResult` (полная сверка результата, заявленного извне, + P2-A `assertNoSegmentLeakage`), `assertOosIsolation`, `assertNoCherryPicking`, `windowOf`.
- `lib/experiment/run.ts` (558) — `runExperiment`: политика → бары (P2-A `validateBars`) → субъект → разбиение (P2-A `chronologicalSplit` + валидатор хронологии) → разрешение конфигураций → для каждой конфигурации ТРИ прогона P2-A `runBacktest` со сверкой идентичности и P2-A-инвариантом утечки ДО принятия результата → порядок представления → три блока свидетельств → ранжирование/выбор → самопроверка инвариантов (`assertOosIsolation`, `assertNoCherryPicking`, пересчёт независимости ранжирования) → замороженная запись. `runExperimentReport` добавляет отчёт сравнения. Стадии отказа эксперимента: `policy`, `bars`, `variants`, `subject`, `split`, `invariant`; отказ отдельной конфигурации эксперимент НЕ останавливает.

**Переиспользование P2-A (второго алгоритма нет).** Импортируются только `lib/backtest/*`: `chronologicalSplit`, `runBacktest`, `assertNoSegmentLeakage`, `SPLIT_NAMES`, `validateBars`, `resolveBacktestConfig`, `BACKTEST_DEFAULTS`, `BACKTEST_CONTRACT_VERSION`, `canonicalJson`, `canonicalNumber`, `sha256Hex`, `fingerprintOf`, `fingerprintConfig`, `fingerprintBars`, `fingerprintResult`, типы (`BacktestBar`, `BacktestResult`, `SegmentWindow`, `SplitName`, `ChronologicalSplit`, …); в тестах — `serializeResult`, `poisonFutureBars`, `entryDecision`, `noTradeDecision`. Собственного разбиения, собственного сериализатора, собственных метрик и собственной семантики исполнения в P2-C нет: P2-A — единственный источник истины. Скан исходников слоя (тест) подтверждает: все импорты — только `../backtest/*` и соседние модули слоя; запрещённые токены (`process.`, `Date.now`, `new Date`, `Math.random`, `randomUUID`, `fetch(`, `@prisma`/`prisma`, `require(`, `setTimeout`, `node:fs`, `worker_threads`, `next/server`, обращения к СУБД/ORM, запись/чтение файлов) отсутствуют.

**Идентичность и отпечатки.** `subjectFingerprint = sha256{strategy(slug, version), market(asset, exchange, timeframe, timeframeMs), dataRange(barsCount, firstBarTime, lastBarTime, barsFingerprint)}`. `configurationId = sha256{subjectFingerprint, label, paramsFingerprint, configFingerprint(P2-A), signalSource}` — то есть идентичность конфигурации привязана к рынку, таймфрейму, диапазону данных, версии стратегии, параметрам-данным, конфигу исполнения P2-A и источнику решений: результат чужого рынка/таймфрейма/конфига имеет ДРУГОЙ `configurationId` и не может быть выдан за свой. Три отпечатка эксперимента: `inputFingerprint` (субъект + разбиение + политики + упорядоченный список идентичностей, БЕЗ результатов), `resultFingerprint` (сегментные результаты P2-A в порядке представления; `inputOrder` сюда намеренно не входит — это характеристика объявления, а не результата), `reportFingerprint = sha256(canonicalJson{scope: "experiment-comparison", record})`. Всё — на P2-A-сериализаторе (сортировка ключей, 10 знаков, −0 → 0, Infinity/Date/функции → исключение).

**Проекция метрик (дословность, пункт 1 политики).** Каждое число сегментного отчёта копируется из `BacktestResult` P2-A поле-в-поле: сделки (всего/LONG/SHORT, wins/losses/breakeven, winRate), деньги (gross/net, суммы выигрышей и убытков net и gross, комиссии, слиппедж, finalEquity), отношения (PF + состояние `ok|no-losses|no-trades`, expectancy, avgR, medianR, avg/median grossR, avgWin/avgLoss, largestWin/largestLoss), обе базы просадки (реализованная и mark-to-market, с временами пика/впадины и флагом `equityNonPositive`), серии (подряд выигрышей/убытков), удержание (avg/max bars), причины выхода (все 5 ключей всегда), `timeoutExits`, `sameBarAmbiguityTrades`, `gapThroughTrades`, `openAtEndTrades`, решения (`signalsEvaluated`, `decisionCounts`, отдельно `cannotEvaluate` / `neutral` / `noSignal` — инвариант «cannot-evaluate ≠ NEUTRAL» сохранён), группировки `skippedByReason` и `rejectedByReason` по кодам P2-A. Единственные «новые» значения — группировки уже имеющихся записей и отпечатки. Тест сверяет 47 числовых полей + счётчики для каждой из 9 комбинаций (3 конфигурации × 3 сегмента) и дополнительно сравнивает отчёт с НЕЗАВИСИМЫМ прогоном P2-A того же сегмента (байт-в-байт `serializeResult`). Sharpe/Sortino не добавлены намеренно: в P2-A их нет, а пересчёт нарушил бы правило «источник истины — P2-A» (заявленное ограничение).

**Порядок представления.** Допустимы ровно две политики: `input-order` (дефолт) и `configuration-id` (лексикографика идентичности). Результат-зависимые порядки (`by-profit-factor`, `by-net-pnl`, `by-win-rate`, `best-first`, `random`) — ошибка валидации на стадии `policy`, ДО каких-либо расчётов. `inputOrder` сохраняется всегда, `presentationOrder` — плотная последовательность 0..N−1 (уникальность и плотность проверяются инвариантом записи). При `configuration-id` порядок объявления не влияет ни на `inputFingerprint`, ни на `resultFingerprint`, ни на состав строк и результатов (отличается только записанный `inputOrder` — происхождение объявления); при `input-order` перестановка объявления меняет порядок строк и отпечатки. Оба свойства проверены тестом.

**Ранжирование и выбор.** По умолчанию — `none`: ничего не ранжируется и не выбирается. `rank-only` фиксирует порядок как свидетельство, НЕ выбирая конфигурацию; `select-by-rank` выбирает вершину явного ранжирования. Стадии — только TRAIN (selection evidence) и VALIDATION (confirmation evidence); `"OOS"` невозможен по типу и отклоняется валидацией политики. Критерии — закрытый набор 9 полей (`trades`, `netPnl`, `winRate`, `profitFactor`, `expectancy`, `avgR`, `medianR`, `maxRealizedDrawdownPct`, `maxConsecutiveLosses`), выведенный компилятором из ключей `EvidenceMetrics`; направления ЗАФИКСИРОВАНЫ константой (`desc` для «чем больше, тем лучше», `asc` для просадки и серий убытков) и не настраиваются вызывающим кодом. `null` (отсутствие свидетельства: нет сделок, нет убытков) всегда сортируется ПОСЛЕДНИМ независимо от направления — «PF = null при нуле убытков» не может победить конечный PF. Полный порядок обеспечивается tie-break по `configurationId`, факт его применения записывается в `tieBreakApplied`. Отклонённые конфигурации в ранжирование не попадают, но перечисляются в `excludedFromRanking` с кодом причины. `selection.performed = true` только если выбор ДЕЙСТВИТЕЛЬНО выполнен (при пустом ранжировании — false, и отчёт не утверждает, что «что-то выбрано»).

**Изоляция OOS (три уровня + перепроверка).**
1. **Структурный:** ранжирование принимает только `EvidenceEntry[]` конкретного блока; блоки TRAIN/VALIDATION/OOS разделены в записи, `SelectionStage` не содержит `"OOS"`, `RankingRecord.oosConsulted` и `SelectionRecord.oosConsulted` имеют тип `false` (иное значение не компилируется).
2. **Политика:** `selectionPolicy.stage = "OOS"` отклоняется валидацией до расчётов; то же — для `ranking.stage`.
3. **Верификация:** `assertRankingIndependentOfOos` пересчитывает записанный порядок из сохранённых свидетельств заявленной стадии и сверяет байт-в-байт (включая список исключённых); результат — `oosIsolation.rankingIndependenceVerified`. Подмена порядка, подделка значений критерия, подстановка OOS-свидетельств вместо TRAIN и флаги `oosConsulted* = true` обнаруживаются (проверено отрицательными контролями).
Эмпирическое доказательство: изменение баров ТОЛЬКО в окне OOS меняет OOS-метрики, OOS-свидетельства, `resultFingerprint` и `reportFingerprint`, но НЕ меняет сделки/метрики/кривую капитала TRAIN и VALIDATION (байт-в-байт), выбранную конфигурацию, порядок ранжирования и значения критериев. Дополнительно: отравление всех баров после границы TRAIN (P2-A `poisonFutureBars`) не меняет результаты TRAIN ни для спискового источника решений, ни для каузального провайдера, — то есть будущее не влияет на прошлое и на уровне эксперимента.

**Принятие результатов и защита от подмен.** Каждый сегментный результат сверяется ДО принятия: сегмент и границы окна (метаданные против разбиения), `barsCount` сегмента, `barsFingerprint` (рынок/набор баров), `timeframeMs` (подмена таймфрейма и «заявлен шаг при неравномерной сетке»), вхождение времён в заявленный диапазон, `contractVersion` (результат, собранный не текущим P2-A, отвергается), `configFingerprint` (результат чужой конфигурации), `configurationId`/`subjectFingerprint` для результатов, заявленных извне, плюс P2-A `assertNoSegmentLeakage` и внутренняя согласованность. При расхождении конфигурация отклоняется с кодом `identity-mismatch` (или `leakage-detected`), а сам результат НЕ публикуется (`result = null`, `report = null`) — «приблизительно верных» чисел слой не выдаёт. Проверено сквозным тестом: субъект с заявленным таймфреймом 15m при часовых барах → все три конфигурации отклонены с `identity-mismatch`, отчёт содержит три строки с причинами, оценённых нет, выбор не выполнен.

**Отсутствие cherry-picking (инвариант записи).** `counts.declared === evaluated + rejected === числу объявленных конфигураций`; `variants.length === declared`; все три блока свидетельств содержат ВСЕ варианты; `presentationOrder` уникален и плотен; у `rejected` всегда есть причина (иначе инвариант краснеет), у `evaluated` всегда есть сегментные результаты; выбранная конфигурация обязана присутствовать среди оценённых («победитель из ниоткуда» невозможен). Доказано на фикстуре 10 объявленных конфигураций → 10 записей и 10 строк отчёта (7 оценено, 3 отклонены с разными причинами: `invalid-config`, `missing-signal-source-id`, `duplicate-configuration-id`), и отрицательным контролем: отчёт «только лучший» валидатор отвергает.

**Детерминизм.** Повторный прогон того же входа даёт БАЙТ-В-БАЙТ идентичную каноническую форму записи и отчёта, идентичные `inputFingerprint`/`resultFingerprint`/`reportFingerprint` и идентичный текстовый вид; идентичность подтверждена и между отдельными процессами node. Никаких `Date.now()`, `randomUUID`, `process.env`, hostname, pid, версий зависимостей и порядка ключей объекта: всё сериализуется P2-A-канонизатором.

**Тесты (три набора, 611 проверок, все зелёные дважды подряд — детерминизм подтверждён повтором):**
- `scripts/test-experiment-contract.ts` — **212/212**: константы и закрытые наборы (версия, слой, provenance, две политики порядка, отсутствие результат-зависимых порядков, стадии выбора без OOS, 9 критериев с фиксированными направлениями, дефолт `none`); диапазон данных из баров (чувствительность к изменению цены, длины, усечению) и 7 отказов `validateSubject`; идентичность конфигурации (воспроизводимость, уникальность, вхождение метки/параметров/конфига/рынка/таймфрейма/диапазона/версии стратегии/источника решений); источник решений (provider без `signalSourceId` отклоняется, список — нет, дескриптор сериализуем); разрешение 8 вариантов (6 отклонений с разными кодами, дубликат — оценивается ПЕРВЫЙ, отказ соседа не прерывает разрешение); порядок представления (2 допустимые политики, 6 запрещённых, лучшая конфигурация НЕ оказывается первой автоматически, лексикографика, детерминированный повтор, отказ на стадии `policy`); валидация политики выбора (допустимые случаи и 7 недопустимых: stage=OOS для обеих политик, неизвестная стадия, критерий `sharpe` вне закрытого набора, дубликат критерия, пустой набор, неизвестный вид политики) плюс отказ прогона на стадии `policy`; ранжирование (направления desc/asc, `null` последний, tie-break по `configurationId` и его независимость от порядка объявления, `tieBreakApplied`, состав участников и исключения); разделённость трёх блоков свидетельств и закрытый набор полей `EvidenceMetrics` (каждый критерий — поле свидетельств; Sharpe/Sortino отсутствуют); 10 → 10 без cherry-picking; 8 отказов уровня эксперимента по стадиям; переиспользование P2-A-разбиения (окна поиндексно, каноническая форма байт-в-байт, warmup в прошлое, пользовательские доли); отпечатки входа/результата/записи и `fingerprintResult` каждого сегмента.
- `scripts/test-experiment-report.ts` — **225/225**: точное соответствие сегментов (метки, окна из разбиения, границы из метаданных, TRAIN ≠ OOS, provenance во всех сегментах); дословность 47 числовых полей + 5 ключей `exitReasonCounts` + 5 ключей `decisionCounts` + группировок skipped/rejected для 9 комбинаций (3 конфигурации × 3 сегмента), `resultFingerprint` = P2-A `fingerprintResult`, сверка с независимым прогоном P2-A и байт-в-байт `serializeResult`, `configFingerprint`; сегмент без сделок (нулевые деньги и просадки, `PF = null` с состоянием `no-trades`, `winRate = null`, 5 ключей причин выхода, вариант остаётся оценённым); состояние без убытков (`PF = null` + `no-losses`, `winRate = 100`, нулевые суммы убытков, отсутствие −0 и Infinity в канонической форме); детерминизм повтора (каноническая запись, отчёт сравнения, три отпечатка, текстовый вид, строки, идентичности, отсутствие следов времени/случайности); реакция отпечатков на изменение конфига/данных/версии стратегии/разбиения/политики (политика меняет `inputFingerprint`, но не `resultFingerprint`); порядок объявления (при `input-order` значим, при `configuration-id` — нет, `inputOrder` сохраняется как перестановка 0..N−1); изменение только OOS (метрики TRAIN/VALIDATION байт-в-байт прежние, OOS изменились, `reportFingerprint` изменился, выбор и порядок ранжирования — нет); семантика отчёта сравнения (счётчики, три блока, соответствие колонок блокам, флаги и механизм изоляции, отклонённые строки с причиной/стадией/текстом); текстовый вид (заголовок, идентичность, границы сегментов, warmup, политика порядка, счётчики, три отпечатка, флаги изоляции, пометка OOS, обоснование выбора, все конфигурации, `PF=null(no-losses)`, `PF=null(no-trades)`, `winRate=n/a`, отсутствие NaN и дат).
- `scripts/test-experiment-leakage.ts` — **174/174**: хронология (реальное разбиение проходит; обнаруживаются пересечение TRAIN/VALIDATION и VALIDATION/OOS, дыра, TRAIN не с нуля, неполное покрытие, пустой сегмент, несовпадение `barsCount`, warmup в будущее, отрицательный warmup, выход за набор баров); каузальный warmup НЕ утечка (`warmupStartIndex = 33` при начале сегмента 36, P2-A-инвариант и идентичность проходят) + отравление будущего (сделки/метрики/кривая TRAIN прежние, метрики OOS изменились, провайдерная конфигурация тоже не изменилась); принятие внешнего результата (подлинный проходит без замечаний; отвергаются TRAIN, выданный за OOS, OOS за VALIDATION, VALIDATION за TRAIN, результат чужой конфигурации, результат, приписанный чужой конфигурации, чужой набор баров, подмена `timeframeMs` и «неравномерная сетка при заявленном шаге», чужой субъект, чужое окно, чужие границы в метаданных, чужая версия контракта, бары вне заявленного диапазона, неизвестное имя сегмента); внутренняя согласованность (завышенное число сделок, подделка `netPnl`, обнулённые причины выхода и wins/losses/breakeven, усечённая кривая, обнулённые счётчики решений, перекрытие позиций); утечки (выход за границей сегмента, сигнал из чужого сегмента, вход на баре выхода предыдущей сделки, запись о пропуске вне окна; легальный перезаход нарушением НЕ считается); изоляция OOS в записи (подлинная запись проходит; обнаруживаются три флага `oosConsulted*`, `ranking.stage = OOS`, подменённый порядок, подделанные значения критерия, подмена списка исключений, подстановка OOS-свидетельств вместо TRAIN, `rankingIndependenceVerified` без ранжирования, выбор несуществующей конфигурации, политика со stage=OOS); cherry-picking (удалённая конфигурация, отчёт «только лучший», завышенный `declared`, `declared ≠ evaluated + rejected`, усечённый блок свидетельств, `rejected` без причины, `evaluated` без результатов, непоследовательный и неуникальный `presentationOrder`; 5 объявлено → 2 оценено + 3 отклонено, все видны в отчёте); сквозная проверка кросс-таймфреймовой подмены самим прогоном (3 конфигурации отклонены с `identity-mismatch`, выбор не выполнен, инварианты соблюдены) и 7 прямых проверок `assertSegmentIdentity`; сквозные инварианты прогона (окна всех прогонов равны окнам разбиения, каждый результат помечен своим сегментом, отпечатки сегментных отчётов совпадают с P2-A, идентичности уникальны, конфиг каждого результата совпадает с конфигом конфигурации, все результаты получены на одних барах); скан изоляции слоя (6 файлов: только разрешённые импорты, отсутствие запрещённых токенов, СУБД/ORM и файловых операций).

**Verification (Arena-песочница; БД, workers, PM2, сеть и `.env` не использовались и не читались):**
- `npx tsx scripts/test-experiment-contract.ts` → **212/212**; `…-report.ts` → **225/225**; `…-leakage.ts` → **174/174**; каждый набор повторно — те же числа (детерминизм). Отпечатки входа/результата/отчёта совпали и между двумя отдельными процессами node.
- P2-A (регрессия обязательная): `test-backtest-engine` **260/260**, `test-backtest-metrics` **123/123**, `test-backtest-splits` **108/108** — без изменений (`lib/backtest/*` не редактировался).
- Прочие регрессии: `test-chart-runtime 81/81`, `test-chart-ux 426/426`, `test-chart-history 50/50`, `test-chart-params 37/37`, `test-smc-lookahead 13/13`, `test-smc-evaluate 31/31`.
- `npx tsc --noEmit` локальным typescript — 0 ошибок на всём репозитории.
- `npm run build` — `✓ Compiled successfully in 3.2s`, проверка типов прошла, далее падение на сборе page-data для `/api/register`: `@prisma/client did not initialize yet` — известное ограничение песочницы (нет сгенерированного клиента и `DATABASE_URL`), идентичное baseline; ни одна страница/API в этом этапе не менялась.
- `git diff --check` — чисто; новых зависимостей нет (`package.json`/`package-lock.json` — 0 изменений); временных файлов в репозитории нет (рабочие скрипты-пробы держались вне репозитория).
- `git merge-base --is-ancestor edf3732da81 HEAD` → exit 1 (Signal Engine не предок).
- БД и workers: ни одного запуска, ни одной записи; `prisma/schema.prisma` — 0 изменений; `Strategy`/`Signal` не тронуты; деплоя не было.

**Известные ограничения (честно, без маркетинга):**
1. P2-C не оптимизатор: перебор/генерация конфигураций, подбор параметров, walk-forward, Monte-Carlo, устойчивость по активам/таймфреймам/режимам рынка — вне этапа. Ранжирование выполняется только по явной политике вызывающего кода.
2. Sharpe/Sortino и любые метрики, которых нет в P2-A, в отчёте отсутствуют намеренно: их пересчёт нарушил бы правило «P2-A — единственный источник истины». Появятся только вместе с расширением P2-A.
3. Идентичность рынка (`asset`, `exchange`, `timeframe`) — ЗАЯВЛЕННЫЕ метаданные: в самих барах тикера нет, поэтому подмену «те же бары, но другой актив» слой отличить не может. Жёстко обеспечивается привязка к фактическим данным (`barsFingerprint`) и к фактическому шагу сетки (`timeframeMs` из метаданных P2-A): расхождение заявленного шага с фактическим отклоняет конфигурацию. В P2-B идентичность актива обязана приходить из БД вместе с барами.
4. Провайдер-форма источника решений несериализуема: идентичность обеспечивается обязательным `signalSourceId`, но СООТВЕТСТВИЕ провайдера этому идентификатору слой не доказывает. Для P2-B это означает обязательный сертификат источника (включая P2-A `assertDecisionInvariance`) до подключения.
5. Конфигурации, отклонённые на этапе разрешения, получают немаркированные идентификаторы вида `unresolved:{порядок входа}` и `{идентичность}#duplicate:{порядок входа}`: это НЕ sha256, а явная метка «идентичность не установлена»/«дубликат». В ранжировании они не участвуют, но в отчёте присутствуют с причиной.
6. Ранжирование — сравнение по заявленным критериям, а не статистический вывод: значимости, доверительных интервалов и поправок на множественное сравнение слой не считает и не обещает. Сравнение многих конфигураций на одном разбиении не заменяет walk-forward; OOS остаётся единственным финальным свидетельством и должен читаться один раз.
7. `inputOrder` входит в каноническую форму записи (происхождение объявления), поэтому при политике `configuration-id` канонические отчёты двух разных порядков объявления различаются именно этим полем; состав строк, результаты и отпечатки входа/результатов при этом идентичны (проверено тестом).
8. Слой работает только в памяти: сохранение отчётов, история прогонов и UI — задача P2-B (нужны БД и отдельное одобрение). Никакой прибыльности этап не утверждает и не доказывает.

**Что дальше.** Интеграция P2-C с реальными данными (P2-B: PostgreSQL/Prisma, runner, сохранение отчётов, UI «Тестер стратегий»), расширение метрик P2-A (Sharpe/Sortino, walk-forward) и сертификация источников сигналов — отдельные этапы. P2-C завершён и СТОП: ничего не развёрнуто, БД не тронута, независимый аудит P2-A продолжается отдельным агентом.

**Files changed (один коммит):** `lib/experiment/contract.ts`, `lib/experiment/identity.ts`, `lib/experiment/report.ts`, `lib/experiment/selection.ts`, `lib/experiment/validate.ts`, `lib/experiment/run.ts` (все — новые), `scripts/test-experiment-contract.ts`, `scripts/test-experiment-report.ts`, `scripts/test-experiment-leakage.ts` (новые), `PROJECT_CONTEXT.md` (этот §44 + статусы P2-A/P2-B/P2-C в roadmap §7).

**Deliverable:** ОДИН логический коммит ровно поверх `096e13d2a5a526d304b0448d987afdc6abf09ce2` (не amend), push только в `arena/01a09406-svechnoy-suslik`. БД/workers/Strategy/деплой/Signal Engine/P2-A не тронуты. После коммита и отчёта — STOP.
