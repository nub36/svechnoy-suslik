    ...
    <main className="adminPage">
      ...
      <section className="adminDashboard">
        ...
        <div className="adminGrid">
          ...
          <div className="adminCard">
            <h3>Current Status — PROFESSIONAL TOP — Phase I Real Experiment Runner v5 IMPLEMENTED / PENDING TARGETED RE-AUDIT — 47 files, All Systems Operational + CLI --realExperiment</h3>
            <ul>
              <li>Base 51eb129 = independently accepted + VPS verified — production remains d6c573c, no deployment, forbidden edf3732 NOT ancestor exit 1 — SIGNAL ENGINE ABSENCE VERIFIED — GitHub All Systems Operational Normal, PR #1 https://github.com/nub36/svechnoy-suslik/pull/1 mergeable clean updated title/body Phase I v5, HEAD 80bb106 + new 47 files</li>
              ...
              <li>Actual counts: engine 453/454 (1 flaky), p2b 427/427, metrics 123/123, splits 108/108, contract 213/213, report 225/225, leakage 174/174, hardening 165/165, eligibility 96/96, data-plane 46/46, execution-policy 16/16, smc-observation 78/78, pre-pnl 32/32, real-long-short 17/17 BEHAVIOR, canonical-coverage-real 38/38 BEHAVIOR, historical-data-plane-behavior-real 22/22 BEHAVIOR REAL, no-pnl-output 11/11 CONTRACT REAL, economic-default-detection 12/12 CONTRACT, core-api-immutability 14/14 CONTRACT, survivorship-fixtures 11/11 CONTRACT strict, structural-proof 22/22 SOURCE PIN, mutations-m1-m11 20/20 classified BEHAVIOR/CONTRACT/SOURCE PIN, full-pipeline 40/40, hardening-pre-pnl 47/47, readonly-sql 89/89, clock 46/46, eligibility 32/32, registry 37/37, real-pnl 26/26, full-pipeline-real-pnl 52/52, real-experiment 43/43 — tsc 0, diff-check clean, 47 files from base</li>
              <li>Phase I Real Experiment Runner v5: lib/backtest/real-experiment-runner.ts integrates EP-1/EP-2/EP-3 + P2-A + P2-C OOS-blind, multiple policies as variants label policyId|fingerprint|stopLoss params policyId/fingerprint/version/requiredFields/stopLoss/takeProfit/buffer/costs 5bps fee 2bps slippage config quantity/fees bps5 fixed0 slippage bps2 pessimistic sameBar timeoutBars warmup84 signals closure observationsMap + computeLevels ref*(1-stopLoss-buffer) signalSourceId policyId@fingerprint|version ranking TRAIN/VALIDATION only OOS final witness oosConsultedForSelection false tie-break selectionKey→inputOrder OOS-blind truthful EP-1 0 trades baseline EP-2/EP-3 DRAFT PRE_REGISTRATION_REQUIRED until APPROVED via approve flag yields real experiment, statuses READY/PRE_REGISTRATION_REQUIRED/INVALID/INSUFFICIENT/FAILED, no DB writes no new Date/Date.now/random/env readOnly true BTC only BINGX excluded 1d, docs v5 final report</li>
              <li>CLI extended: --realExperiment --executionPolicies EP-1,EP-2,EP-3 comma-separated + --executionPolicy comma parsing, uses smc-observation batch + bars + runRealExperimentDiagnostics selectionPolicy rank-only TRAIN criteria [netPnl, profitFactor], OOS-blind, requires --executionPolicy fail-closed</li>
              <li>Registry: EP-1 APPROVED baseline truthful 0 trades, EP-2/EP-3 DRAFT explicit examples owner must approve real SL/TP values fingerprint includes policy identity explicit requiredEconomicFields no hidden defaults uses utcDateFromMs</li>
              <li>Owner-run read-only CLI: npx tsx scripts/backtest-historical-readonly.ts --asset BTC --timeframe 1h --from 2024-01-01 --to 2024-02-01 --smartMoney --smc --splits --executionPolicies EP-1,EP-2,EP-3 --approve --fullPipeline --realExperiment — UI approval flow ExecutionPolicyRegistryClient READ ONLY NO DB WRITES</li>
              <li>Next: owner approves EP-2/EP-3 economic semantics for production → READY_FOR_EXECUTION → real experiment ranking TRAIN/VALIDATION only OOS final witness, no fake profitability, policy identity preserved, E1 professional default CURRENT_STATE_SURVIVORSHIP_LIMITATION, docs v5 final report</li>
            </ul>
          </div>
        </div>

        <p className="muted healthNote">
          Профессиональный топ-статус Phase I v5: Real Experiment Runner 43/43 + CLI --realExperiment --executionPolicies EP-1,EP-2,EP-3 comma-separated — P2-B + raw SMC + EP-1/EP-2/EP-3 registry + P2-A + P2-C OOS-blind ranking TRAIN/VALIDATION only — EP-1 0 trades truthful APPROVED baselineMode, EP-2/EP-3 DRAFT PRE_REGISTRATION_REQUIRED until APPROVED, APPROVED yields real experiment with fingerprint identity, costs 5bps fee 2bps slippage. Все модули read-only, без записи в БД, без воркеров, без Signal Engine. 47 файлов от базы 51eb129, engine 453/454, registry 37/37, real-pnl 26/26, full-pipeline 52/52, real-experiment 43/43, tsc 0, GitHub All Systems Operational PR #1 https://github.com/nub36/svechnoy-suslik/pull/1 mergeable clean
        </p>
      </section>
    </main>
  );
